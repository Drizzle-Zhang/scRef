  ...
  SINGLE <- function(i) {
    ...
    while (j <= length(colname_ref)) {
      ...
      if (method == 'kendall') {
        ...
        if (method == 'cosine') {
          log_p_sc_given_ref <- sum(exp_sc * exp_ref) / sqrt(sum(exp_sc^2) * sum(exp_ref^2))
        } else {
...
.find_markers <- function(exp_ref_mat, type_ref = 'count', topN = NULL, cutoff.fc = NULL, cutoff.pval = NULL) {
...
.find_markers_auto1 <- function(exp_ref_mat, type = 'count', topN = NULL, 
                                cutoff.fc = NULL, cutoff.pval = NULL) {
  ###### regard MCA as reference of DEG
  file.MCA <- '/home/disk/scRef/MouseAtlas_SingleCell_Han2018/combinedMCA/MCA_combined_mouse_uniform.txt'
...
.find_markers_auto <- function(exp_ref_mat, type_ref = 'count', out.group = 'MCA',
  ...
  for (i in 1:dim(out)[1]) {
    ...
    sel.names <- MCA.names[order(out[i,], decreasing = T)[1]]
    print(paste0('Reference cells: ', ref.names[i], ' | ', 'Matched cells: ', sel.names))
...
.cluster_sc <- function(exp_sc_mat) {
  ...
  seurat.unlabeled <- FindVariableFeatures(seurat.unlabeled, selection.method = "vst", 
  ...
  # cluster
  seurat.unlabeled <- FindNeighbors(seurat.unlabeled, reduction = "pca", dims = 1:75, 
                                    nn.eps = 0.5, verbose = F)
  seurat.unlabeled <- FindClusters(seurat.unlabeled, resolution = 3, n.start = 20, 
                                   n.iter = 100, verbose = F)
  
  out.cluster <- data.frame(cluster.id = as.character(seurat.unlabeled@meta.data$seurat_clusters),
...
.cluster_increase_speed <- function(exp_sc_mat, df.cluster, cluster.cell = 5) {
  ...
  merge.one.cluster <- function(cluster.id) {
    ...
    sub.seurat <- NormalizeData(sub.seurat, normalization.method = "LogNormalize", 
                                scale.factor = 10000, verbose = F)
    # print(sub.seurat@assays$RNA@data[1:6,1:6])
    sub.seurat <- FindVariableFeatures(sub.seurat, selection.method = "vst", 
    ...
    res.cluster <- kmeans(sub.pca, centers = num.clusters, nstart = 20, iter.max = 50)

    # names
    sub.dict <- data.frame(cell.id = names(res.cluster$cluster), 
                           cluster.level1 = rep(cluster.id, num.cell),
                           cluster.level2 = res.cluster$cluster)
    sub.dict$cluster.merge.id <- paste(sub.dict$cluster.level1, sub.dict$cluster.level2, sep = '-')
...
SCREF <- function(exp_sc_mat, exp_ref_mat, type_ref = 'count', out.group = 'MCA', 
  ...
  if (!type_ref %in% c('count', 'fpkm', 'tpm')) {
    print('Error: inexistent input of reference data format')
    return()
  }
  ...
  out.markers <- .find_markers_auto(exp_ref_mat, type_ref = type_ref, topN = 100)
  ...
  if (cluster.speed) {
    ...
    # time1 <- Sys.time()
    out.merge <- .cluster_increase_speed(exp_sc_mat, df.cluster, cluster.cell = cluster.cell)
    # time2 <- Sys.time()
  ...
  if (cutoff.1 == 'default') {
    tag.base <- data.frame(cell_id = names(list.cell.genes), tag = names(list.cell.genes))
  ...
  tag.final <- data.frame(scRef.tag.12 = tag.final, pvalue = pval.final, 
  ...
  if (cutoff.2 == 'default') {
    ...
    for (cluster.id in cluster.ids) {
      ...
      min.log10Pval <- min(sub.tag.cluster[sub.tag.cluster$scRef.tag.12 == main.cell, 'log10Pval'])
  ...
  print('Default cutoff: ')
  print(cutoff.2)
  scRef.tag <- df.tags$scRef.tag.12
