path.tworound <- '/home/zy/scRef/two_round/'
...
path.ben <- '/home/zy/scRef/Benchmark/'
path.tworound <- '/home/zy/scRef/two_round/'
df.plot <- data.frame()
...
df.plot <- rbind(df.plot, data.frame(strategy = 'single round', data = '4th situation',
...
df.plot <- rbind(df.plot, data.frame(strategy = 'two round', data = '4th situation',
                                     Dataset = 'Han -> Ding, Human PBMC',
                                     accuracy = res.two.4$accuracy))
df.plot$label <- paste(df.plot$Dataset, df.plot$data, sep = ' (')
df.plot$label <- paste0(df.plot$label, ')')
df.plot$label <- factor(df.plot$label, levels = rev(c('Muraro -> Segerstolpe, Human pancreas (1st situation)',
