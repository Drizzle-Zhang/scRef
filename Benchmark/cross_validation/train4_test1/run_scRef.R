run_scRef<-function(DataPath,LabelsPath,CV_RDataPath,OutputDir,GeneOrderPath = NULL,NumGenes = NULL){
  ...
  for (i in c(1:n_folds)){
    ...
    start_time <- Sys.time()
    set.seed(1234)
    REF <- .generate_ref(sce_cpm, cbind(as.character(sce_metadata[,1]),as.character(sce_metadata[,1])),min_cell=1)
    ...
    # scPred Prediction
    start_time <- Sys.time()
    scp <- SCREF(sce_cpm_test,REF,min_cell=1,CPU=4)
    #scp=.get_log_p_sc_given_ref(sce_counts_test, REF, CPU=1)
    #scp=.get_tag_max(scp)
    ...
    Pred_Labels_scPred[i] <- list(scp$tag2[,2])#list(scp[,2])
  }
  ...
  if(!is.null(GeneOrderPath) & !is.null (NumGenes)){
    write.csv(True_Labels_scPred,paste('scRef_',NumGenes,'_True_Labels.csv', sep = ''),row.names = FALSE)
    write.csv(Pred_Labels_scPred,paste('scRef_',NumGenes,'_Pred_Labels.csv', sep = ''),row.names = FALSE)
    write.csv(Training_Time_scPred,paste('scRef_',NumGenes,'_Training_Time.csv', sep = ''),row.names = FALSE)
    write.csv(Testing_Time_scPred,paste('scRef_',NumGenes,'_Testing_Time.csv', sep = ''),row.names = FALSE)
  }
  else{
    write.csv(True_Labels_scPred,'scRef_True_Labels.csv',row.names = FALSE)
    write.csv(Pred_Labels_scPred,'scRef_Pred_Labels.csv',row.names = FALSE)
    write.csv(Training_Time_scPred,'scRef_Training_Time.csv',row.names = FALSE)
    write.csv(Testing_Time_scPred,'scRef_Testing_Time.csv',row.names = FALSE)
  }
}
